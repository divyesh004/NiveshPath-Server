...
const nextQuestionMap = {
  ...
  demographicInfo: "Can you provide more information about yourself?",
  psychologicalProfile: "How do you make investment decisions?"
};
...
const askNextQuestion = (missingFields, askedQuestions = []) => {
  ...
  return nextQuestionMap[nextField] || `Can you add ${nextField} to your profile?`;
};

// We no longer need these functions as dynamic questions are now generated by Mistral AI
// Helper function to shuffle array (keeping this as it might be used elsewhere)
...
exports.submitQuery = async (req, res, next) => {
  try {
    ...
    // We no longer need to append follow-up questions as they are now generated by Mistral AI
    // The dynamic follow-up questions are now part of the AI's response
...
Provide a DETAILED COMPARISON between direct stock investments and mutual funds specifically tailored to this user's situation. FORMAT YOUR RESPONSE: 1) Create a comprehensive comparison TABLE with rows for: control & decision making, diversification, research required, time commitment, minimum investment, expense ratio/costs, tax implications, and recommended allocation based on user profile. 2) Include a PERSONALIZED RECOMMENDATION section stating whether they should prefer direct stocks, mutual funds, or a hybrid approach with EXACT percentages and SPECIFIC investment vehicles. 3) Include a KNOWLEDGE ASSESSMENT section that evaluates their likely investment knowledge level based on their profile and tailors recommendations accordingly. 4) Provide a GETTING STARTED section with exact steps to begin implementing your recommendation. Use a conversational, friendly tone and tailor your communication style to match their psychological profile.)`;
    ...
    // Function to generate dynamic follow-up questions based on the response
    function generateDynamicQuestions(responseText, context, query) {
      // Default questions based on common financial topics
      ...
      // Questions based on user profile status
      if (context.profileStatus && !context.profileStatus.complete) {
        // Add profile completion questions
        context.profileStatus.missingFields.forEach(field => {
          if (nextQuestionMap[field]) {
            defaultQuestions.push(nextQuestionMap[field]);
          }
        });
      }
      
      // Questions based on query context
      if (context.isInvestmentQuery) {
        defaultQuestions.push(
          "Are mutual funds safe?",
          "SIP vs lump sum investment, which is better?",
          "Which investments are good for new investors?"
        );
      }
      ...
      return shuffleArray(defaultQuestions).slice(0, 3);
    }
    
    // Get dynamic questions
    const dynamicQuestions = generateDynamicQuestions(responseText, context, query);
    
    // Append dynamic questions to the response
